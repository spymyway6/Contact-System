<!-- jQuery  -->
<script src="<?=base_url();?>assets/js/jquery.min.js" type="text/javascript"></script>
<script src="<?=base_url();?>assets/plugins/parsleyjs/dist/parsley.min.js" type="text/javascript"></script>
<script src="<?=base_url();?>assets/plugins/sweetalert/sweetalert.min.js" type="text/javascript"></script>
<script src="<?=base_url();?>assets/js/init_contacts.js" type="text/javascript"></script>
<script src="<?=base_url();?>assets/js/init_profile_settings.js" type="text/javascript"></script>